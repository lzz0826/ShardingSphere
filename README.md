...
| 維度     | 說明                    | 解決的問題           |
| ------ | ------------------------------ | ---------------------- |
| 垂直拆分 (Vertical)   | 按業務模組拆分。例如：將用戶表、訂單表、商品表分別放在不同的資料庫。             | 解決「單一數據庫連線數過多」與「業務耦合」問題。  |
| 水平拆分 (Horizontal)   | 按數據行 (Row) 拆分。將同一張大表的數據，按規則分散到多個小表或多個庫中。                   | 解決「單表數據量過大」導致索引失效、查詢變慢的問題。           |
